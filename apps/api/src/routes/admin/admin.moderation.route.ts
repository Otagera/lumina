...
import { clearAllPendingService } from "../../services/admin/clearAllPending.service.ts";
...
const adminModerationRoutes = new Elysia({ prefix: "/moderation" })
	...
	.delete("/pending", async ({ adminUser, set }) => {
		try {
			const data = await clearAllPendingService();
			set.status = HTTP_STATUS_CODES.OK;
			return { status: "completed", message: `Cleared ${data.count} pending image(s).`, data };
		} catch (error: any) {
			set.status = error?.statusCode ?? HTTP_STATUS_CODES.INTERNAL_SERVER_ERROR;
			return { status: "error", message: error?.message ?? "Internal server error", data: null };
		}
