...
function validateService(filePath: string): ValidationResult {
	...
	if (
		!content.match(/joi\s*\.\s*object/i) &&
		!content.match(/Joi\s*\.\s*object/i)
	) {
...
function validateRoute(filePath: string): ValidationResult {
	...
	if (
		content.includes("packages/models/src") ||
		content.includes('from "../../../../packages/models')
	) {
...
function main() {
	...
	const actualErrors = invalid.filter(
		(r) => !r.errors.every((e) => e.includes("lines")),
	);
