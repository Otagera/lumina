...
erDiagram
    ...
    albums {
        ...
        UUID    source_album_id FK "self-ref → delivered gallery"
        TEXT    share_token     UK
        TEXT    shared_link
        TEXT    qr_color
        TEXT    qr_logo_url
    ...
    album_images {
        ...
        UUID    album_id        FK
        UUID    image_id        FK
        INT     position        "nullable; used for delivered gallery ordering"
    }
    ...
    album_views {
        ...
        UUID    album_id
        TEXT    session_hash
        TEXT    view_type       "page_view | selfie_search | text_search"
        TSTZ    created_at
    }
    ...
    album_highlights {
        UUID    id              PK
        UUID    album_id        UK
        JSONB   image_ids       "ordered array of image UUIDs"
    ...
    albums                  ||--o{ album_views            : "tracked by"
    albums                  ||--o| album_highlights       : "has highlights"
    albums                  }o--o| albums                 : "delivered gallery of"
...
- `album_settings.allow_downloads` — `BOOL DEFAULT true`; guests can only download when album phase is `delivered`
- `album_settings.display_pin` — 4-digit PIN for Live Display mode; nullable until generated by host
- `album_images.position` — `INT NULL`; set by host drag-reorder in delivered gallery; `NULL` items sort after positioned ones
- `albums.source_album_id` — self-referential FK; non-null on delivered gallery albums, points to the parent event album
- `album_views.view_type` — `page_view` (shared album load) | `selfie_search` | `text_search`; no FK on `album_id` (lightweight, no cascade)
- `album_highlights.image_ids` — `JSONB` ordered array of `image_id` UUIDs; order is the curator's chosen story arc
